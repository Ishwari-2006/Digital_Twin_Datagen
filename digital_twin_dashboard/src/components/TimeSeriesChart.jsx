...
const COLORS = { ice: '#6fd4e8', aurora: '#6de6b0', amber: '#f0b94a', flare: '#ff7a5c' }
    ...
    <div className="ts-tooltip">
      ...
      {payload.map((p) => (
        <div key={p.dataKey} className="ts-tooltip__row">
          <span className="ts-tooltip__swatch" style={{ background: p.color }} />
          {p.name}: <strong className="num">{typeof p.value === 'number' ? p.value.toFixed(1) : p.value}{unit}</strong>
        </div>
      ))}
    </div>
  )
 ...
 */
export default function TimeSeriesChart({ data, series, unit = '', height = 190 }) {
  if (!data || data.length < 2) {
    return <div className="ts-empty">Not enough history yet — check back in a few minutes.</div>
  }
  return (
